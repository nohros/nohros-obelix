...
'**
'* Refresh the underlying database. This method is called when a database refresh is desired.
'*
Public Function GetOutData(ByVal obelix_sqlite_db As ObelixSQLite) As Boolean
    Dim data_range As Range
    Dim first_data_row As Range
    Dim no_of_rows As Long
    Dim no_of_columns As Long
    ...
    main.LoadData obelix_sqlite_db
       
    ' refresh the [agents_table] table.
    
    GoTo Finally
    
Catch:
    LogError "[ObelixTemplates RefreshDatabase]   " & Err.Description
